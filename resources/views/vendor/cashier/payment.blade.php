...
<head>
    ...
    <title>{{ trans('misc.confirm_payment') }} - {{ $settings->title }}</title>
    ...
    <div id="app" class="h-full md:flex md:justify-center md:items-center">
        <div class="w-full max-w-lg">
            <h1 class="text-4xl font-bold text-center p-4 sm:p-6 mt-4">
                {{ trans('misc.your_amount_payment', ['amount' => $amount]) }}
            </h1>
            ...
            <div class="bg-white rounded-lg shadow-xl p-4 sm:p-6 mt-4">
                <div v-if="paymentIntent.status === 'succeeded'">
                    <h2 class="text-xl mb-4 text-gray-600">
                        {{ trans('misc.payment_successful') }}
                    </h2>

                    <p class="mb-6">
                        {{ trans('misc.payment_already_successfully_confirmed') }}
                    </p>
                </div>

                <div v-else-if="paymentIntent.status === 'processing'">
                    <h2 class="text-xl mb-4 text-gray-600">
                        {{ trans('misc.payment_processing') }}
                    </h2>

                    <p class="mb-6">
                      {{ trans('misc.payment_processing_info') }}
                    </p>
                </div>

                <div v-else-if="paymentIntent.status === 'canceled'">
                    <h2 class="text-xl mb-4 text-gray-600">
                        {{ trans('misc.payment_cancelled') }}
                    </h2>

                    <p class="mb-6">
                        {{ trans('misc.payment_was_cancelled') }}
                    </p>
                </div>
                ...
                <div v-else>
                    ...
                    <div v-if="paymentIntent.status === 'requires_payment_method'" class="mb-3">
                        ...
                        <div v-else>
                            <p class="text-sm mb-3">
                                {{ trans('misc.stripe_text_info_1') }} @{{ paymentMethodTitle }}.
                            </p>
                        </div>
                    ...
                    >
                        ...
                        <span v-else>
                          {{ trans('misc.stripe_text_info_4', ['amount' => $amount]) }} @{{ paymentMethodTitle }}                        </span>
                    </button>
                </div>

                <button @click="goBack" ref="goBackButton" data-redirect="{{ url('account/subscription') }}"
            ...
            <p class="text-center text-gray-500 text-sm mt-4 pb-4">
                © {{ date('Y') }} {{ config('app.name') }}. {{trans('emails.rights_reserved')}}
            </p>
        </div>
    </div>
                ...
                confirmCallback: function (result) {
                    ...
                    if (result.error) {
                        if (result.error.code === '{{ Stripe\ErrorObject::CODE_PARAMETER_INVALID_EMPTY }}') {
                            this.errorMessage = '{{ trans('misc.stripe_text_info_5') }}';
                        } else {
