      ...
      <div class="alert alert-success alert-dismissible fade show" role="alert">
            ...
            @foreach ($data as $withdrawal)
                   ...
                   <tr>
                     <td>{{ $withdrawal->id }}</td>
                     <td>
                       <a href="{{ url($withdrawal->user()->username) }}" target="_blank">{{ $withdrawal->user()->username }} <i class="bi-box-arrow-up-right"></i></a>
                       </td>
                     ...
                     <td>
                       ...
                       @else
                       <span class="badge bg-warning">{{trans('misc.pending_to_pay')}}</span>
                       @endif
                     </td>
                     <td>{{ date('d M, Y', strtotime($withdrawal->date)) }}</td>
                     <td>

                       <a href="{{ url('panel/admin/withdrawal',$withdrawal->id) }}" class="btn btn-1 btn-sm btn-dark" title="{{trans('admin.view')}}">
