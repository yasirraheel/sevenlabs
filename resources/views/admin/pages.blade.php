                  ...
                  @if ($data->count() !=  0)
                    ...
                    @foreach ($data as $page)
                      <tr>
                        ...
                        <td>
                        	<a href="{{ url('panel/admin/pages/edit', $page->id) }}" class="text-reset fs-5 me-2">
                        		<i class="far fa-edit"></i>
                        	</a>

                       @if ($data->count() != 1)
							<form action="{{ route('pages.destroy', $page->id) }}" method="POST" id="form{{ $page->id }}" class="d-inline-block align-top">
								@csrf
								<button type="button" data-url="{{ $page->id }}" class="btn btn-link text-danger e-none fs-5 p-0 actionDelete">
