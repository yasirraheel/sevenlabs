...
@section('content')
    ...
    <a class="text-reset" href="{{ url('panel/admin') }}">{{ __('admin.dashboard') }}</a>
      ...
      <span class="text-muted">{{ __('admin.members_reported') }} ({{$data->count()}})</span>
  </h5>
      ...
      <div class="alert alert-success alert-dismissible fade show" role="alert">
			...
			<div class="card shadow-custom border-0">
               ...
               @if ($data->count() !=  0)
                 ...
                 @foreach ($data as $report)
                   <tr>
                     ...
                     <td><a href="{{ url($report->user_reported()->username) }}" target="_blank">{{ $report->user_reported()->username }} <i class="bi-box-arrow-up-right"></i></a></td>
               ...
               $reason = trans('admin.spoofing');
                     ...
                     <td>
                      <form action="{{ url('panel/admin/members-reported') }}" method="POST" class="displayInline">
                        @csrf
