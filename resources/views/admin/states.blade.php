...
@section('content')
    ...
    <a class="text-reset" href="{{ url('panel/admin') }}">{{ __('admin.dashboard') }}</a>
      ...
      <span class="text-muted">{{ __('admin.states') }}</span>
							...
							@if ($states->count() !=  0)
								 <tr>
									  <th class="active">ID</th>
										...
										<th class="active">{{ trans('misc.state') }}</th>
										<th class="active">{{ trans('misc.iso_code') }}</th>
								...
								@foreach ($states as $state)
									<tr>
										...
										<td>{{ $state->code }}</td>
										<td>
											<a href="{{ url('panel/admin/states/edit', $state->id) }}" class="text-reset fs-5 me-2">
												<i class="far fa-edit"></i>
											</a>

											<form action="{{ url('panel/admin/states/delete', $state->id) }}" method="POST" class="d-inline-block align-top">
												@csrf
