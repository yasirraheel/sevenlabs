  			...
  			<div class="container">
  				<div class="row">
  					<div class="col-md-12 error-page text-center parallax-fade-top" style="top: 0px; opacity: 1;">
  						<h1>404</h1>
  						<p class="mt-3 mb-5">{{ __('error.error_404_description') }}</p>
  						<a href="javascript:history.back();" class="error-link mt-5"><i class="fa fa-long-arrow-alt-left mr-2"></i> {{ __('auth.back') }}</a>
						  <br>
						  <a href="{{route('home')}}" class="error-link mt-5">{{ __('error.go_home') }}</a>
  					</div>
  				</div>
  			</div>
  		</div>
